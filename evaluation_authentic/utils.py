...
def load(user):
  def graph(A):
    ...
    return G
  reg=np.load('./dataset/authentic/50/test/'+user+'_50.npy').astype(int)[:2400]
  gen=np.load('./validation_data_auth_50/'+user+'_gen50.npy').astype(int)[:,:,:,0]
  mal=np.load('./dataset/authentic/50/test/'+user+'_mal50.npy').astype(int)
  Gs_reg=[]
  Gs_gen=[]
  Gs_mal=[]
...
def compute_nystrom(user,use_node_labels, embedding_dim, community_detection_method, kernels):
   graphs_reg, labels_reg,graphs_gen, labels_gen,graphs_mal, labels_mal = load(user)
