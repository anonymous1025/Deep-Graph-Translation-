...
def load():
  def graph(A):
    ...
    return G
  reg=np.load('./dataset/poisson random/10/'+'poisson_reg10.npy').astype(int)[2500:]
  gen=np.load('./poisson_gen10.npy').astype(int)[:,:,:,0]   #upload the generated graphs
  mal=np.load('./dataset/poisson random/10/'+'poisson_mal10.npy').astype(int)[2500:]
  Gs_reg=[]
  Gs_gen=[]
  Gs_mal=[]
